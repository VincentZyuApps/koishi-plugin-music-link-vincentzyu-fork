...
function renderSongListText(formattedList, config, options = {}) {
    ...
    const finalWaitTimeText = waitTimeText || (session ? session.text(`.waitTime`, [waitTimeout, atRobotText]) : `⏰ 请在${waitTimeout}秒内输入歌曲对应的序号`);

    if (useRealAtRobot && finalWaitTimeText.includes(atRobotText) && atRobotSegment) {
        const parts = finalWaitTimeText.split(atRobotText);
        ...
        messageSegments.push(h.text(finalWaitTimeText));
    }
