...
function registerFont(fontPath, alias) {
    ...
    try {
        ...
        console.warn(`[renderer-canvas] Failed to register font: ${fontPath}`, e.message);
        return false;
    }
}
...
function renderSongListCanvas(songs, options = {}) {
    ...
    for (let i = 0; i < songs.length; i++) {
        ...
        ctx.fillStyle = subTextColor;
        ctx.fillText(truncate(artist, 21), labelX, y + 28 + metaYOffset);

        const albumStr = song.album?.name || song.albumName || song.album;
        if (albumStr) {
            ctx.font = buildFontSpec(10, familyList);
            ctx.fillStyle = subTextColor;
            ctx.fillText(truncate(albumStr, 25), labelX, y + 38 + metaYOffset);
        }
    ...
    return {
        buffer,
        renderInfo: showRenderInfo ? `(\u{1F3A8} canvas\u6E32\u67D3\u8017\u65F6\uFF1A${elapsed}ms | scale: ${internalScale}x)\n\n` : null,
    };
}
