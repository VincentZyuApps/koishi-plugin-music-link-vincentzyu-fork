...
exports.normalizeAssetConfigPaths = exports.getDefaultAssetDisplayPath = exports.getRuntimeAssetPath = exports.getRuntimeAssetsDir = exports.validateAssets = exports.createUtilFunctions = exports.safeUnlink = exports.getExtensionFromContentType = void 0;
...
const ASSET_CONFIGS = [
    ...
    },
    {
        ...
        sha256: '846efc02fccf6ad73077b7e4f2a879f70dae0db9513b5893d5e982f76f17ac83',
        size: 2010978,
        type: 'image',
    },
    {
        filename: 'pixai_koishi.png',
        sha256: '8b02b8efe6acbf935ea1c90e5bc00cd76592cef931af0cde4db03db4dc9ef380',
        size: 450187,
        type: 'image',
    }
];
...
function getAssetConfig(filename) {
    return ASSET_CONFIGS.find(item => item.filename === filename);
}
...
async function verifyAssetFile(filePath, assetConfig) {
    try {
        ...
        return {
            ok: actualSize === assetConfig.size && actualSha256 === assetConfig.sha256,
            size: actualSize,
            sha256: actualSha256,
        };
    } catch (error) {
        if (error.code === 'ENOENT') {
            return { ok: false, missing: true, size: 0, sha256: '' };
        }
        throw error;
    }
}
...
function normalizeAssetConfigPaths(ctx, config, logger) {
    if (!config) return;
    const mappings = [
    ...
    for (const [key, filename] of arrayMappings) {
        ...
        config[key] = config[key].map(item => (
            isDefaultOrLegacyAssetPath(item, filename)
                ? getRuntimeAssetPath(ctx.baseDir, filename)
                : item
        ));
    }

    config.__musicLinkDefaultSourceHanSerifPath = getRuntimeAssetPath(ctx.baseDir, 'SourceHanSerifSC-Medium.otf');
    logger?.info?.(`📁 [资源路径] 默认资源目录: ${getRuntimeAssetsDir(ctx.baseDir)}`);
}
exports.normalizeAssetConfigPaths = normalizeAssetConfigPaths;
...
async function validateAssets(ctx, logger) {
    ...
    for (const assetConfig of ASSET_CONFIGS) {
        ...
        if (existingCheck.ok) {
            logger.debug(`✅ [资源校验] ${typeLabel} ${assetConfig.filename} 已存在且 sha256 校验通过`);
            downloadStatus.push({ name: assetConfig.filename, status: 'exists', type: typeLabel });
            continue;
        }

        if (existingCheck.missing) {
            logger.info(`📦 [资源下载] ${typeLabel} ${assetConfig.filename} 不存在，开始下载...`);
        } else {
            logger.warn(`⚠️ [资源校验] ${typeLabel} ${assetConfig.filename} 校验失败，准备重新下载`);
            logger.warn(`   ├─ size: ${existingCheck.size} / ${assetConfig.size}`);
            logger.warn(`   └─ sha256: ${existingCheck.sha256 || '(empty)'} / ${assetConfig.sha256}`);
        }

        let lastError = null;
        for (const candidate of buildAssetUrls(assetConfig.filename)) {
            try {
                logger.info(`🌐 [资源下载] 正在尝试 ${candidate.source}: ${candidate.url}`);
                ...
                if (!finalCheck.ok) {
                    throw new Error(`写入后校验失败: size=${finalCheck.size}/${assetConfig.size}, sha256=${finalCheck.sha256}/${assetConfig.sha256}`);
                }
                logger.info(`✅ [资源下载] ${typeLabel} ${assetConfig.filename} 已从 ${candidate.source} 下载并通过 sha256 校验`);
                downloadStatus.push({ name: assetConfig.filename, status: 'success', type: typeLabel });
                lastError = null;
                break;
            } catch (error) {
                lastError = error;
                logger.warn(`⚠️ [资源下载] ${candidate.source} 下载 ${assetConfig.filename} 失败: ${error.message}`);
            }
        }

        if (lastError) {
            logger.error(`❌ [资源下载] ${typeLabel} ${assetConfig.filename} 在 Gitee / GitHub 均下载或校验失败`);
