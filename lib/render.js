...
async function generateSongListImage(pptr, listText, config, logger, imageStyle, songList) {
    ...
    if (useSvg && songList && Array.isArray(songList)) {
        try {
            ...
            if (config.svgShowRenderInfo) {
                renderInfo = svgResult.renderInfo;
            }
            
            return { buffer: svgResult.buffer, renderInfo, isSvg: true };
        } catch (error) {
    ...
    if (usePuppeteer) {
        try {
            return await renderSongListPuppeteer(pptr, listText, config, logger, imageStyle);
        } catch (error) {
