   ...
   ctx.on('ready', async () => {
            ...
            .action(async ({ session, options }, keyword) => {
               ...
               if (isSongId && !options.number) {
                  try {
                     ...
                     const response = generateResponse(session, processedSongData, config.command6_return_data_Field, 'netease');
                     return response;
                  } catch (error) {
      ...
      async function generateResponse(session, data, platformconfig, platform = '') {
         ...
         if (config.command6_add_music_card) {
            ...
            logInfo(`command6,网易api，即将发送发送onebot音乐卡片消息：${JSON.stringify(musicCardJson)}`);
            onebotBot && await onebotBot._request(
               'send_group_msg',
               {
                  "group_id": session.channelId,
                  "message": [ musicCardJson ]
               }
            )
         }
