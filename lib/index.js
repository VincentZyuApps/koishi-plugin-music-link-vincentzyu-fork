...
function apply(ctx, config) {
    ...
    async function renderSongList(ctx, session, config, logger, options, songList, formattedList) {
        ...
        if (!useText && !usePuppeteer && !useSvg && !useMarkdownTable && !useMarkdownStyle) {
            return `${config.enableQuote ? h.quote(session.messageId) : ''}❌ 你没有勾选任何格式的歌单！请去 WebUI 渲染模式设置 勾选至少一个 renderMode`;
        }

        const selectedNoneFallbackTip = '⚠️\t  检测到未选择任何渲染模式，已自动 fallback 到 单个SVG\n';
        selectedNone && logger.warn(selectedNoneFallbackTip);
        // 发送统一的渲染提示（如果启用）
        let waitTipMsgId = null; //默认值是falsy值呢
        if (config.showGeneratingTip !== false) {
            waitTipMsgId = await session.send(`${config.enableQuote ? h.quote(session.messageId) : ''}${selectedNone ? selectedNoneFallbackTip : ''}🔄 正在生成歌单，请稍候⏳...`);
        }
