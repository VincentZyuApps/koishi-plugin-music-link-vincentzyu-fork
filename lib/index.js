...
function apply(ctx, config) {
    ...
    function parseRenderMode() {
        ...
        return { useText, usePuppeteer, useSvg, useMarkdownTable, useMarkdownStyle, order: enabledModes, selectedNone };
    }

    // 启动时显示配置提示
    const { useText, usePuppeteer, useSvg, useMarkdownTable, useMarkdownStyle, order, selectedNone } = parseRenderMode();
    ...
    // 使用 notifier 显示插件信息
    createNotifierInfoOfConfig(ctx, config, { useText, usePuppeteer, useSvg, useMarkdownTable, useMarkdownStyle, order, selectedNone });
    ...
    // 创建工具函数集合
    const { ensureCacheDir, requestWithProxy, downloadFile, sendMusicCard, isCrackAdapter, sendQQFile } = createUtilFunctions({
        ctx,
        config,
        logger,
        logInfo,
    ...
    async function generateResponse(session, data, platformconfig, platform = '') {
        ...
        for (const field of platformconfig) {
                ...
                case 'file':
                    try {
                        ...
                        if (fileInfo) {
                            ...
                            // QQ 平台特殊处理：直接发送文件，不推入 fileElements
                            if (session.platform === 'qq') {
                                let fileUrl;
                                if (config.fileTransferMode === 'base64') {
                                    fileUrl = `data:${fileInfo.mimeType};base64,${fileInfo.base64}`;
                                } else {
                                    fileUrl = url.pathToFileURL(fileInfo.localPath).href;
                                ...
                                logInfo(`📤 [QQ文件发送] 文件名: ${finalFilename}, mimeType: ${fileInfo.mimeType}`);
                                const qqFileElement = await sendQQFile(ctx, session, fileUrl, finalFilename);
                                if (qqFileElement) {
                                    // crack 适配器返回 h.file() 元素，需要立即发送
                                    await session.send(qqFileElement);
                                    logInfo(`✅ [QQ文件发送] 成功发送文件: ${finalFilename}`);
                                }
                                // official 适配器在 sendQQFile 内部直接发送，不返回元素
                            } else {
                                // 非 QQ 平台：保持原逻辑，推入 fileElements
                                if (config.fileTransferMode === 'base64') {
            ...
            if (config.dataFieldSortMode === 'raw' && element) {
    ...
    async function renderSongList(ctx, session, config, logger, options, songList, formattedList) {
        ...
        selectedNone && logger.warn(selectedNoneFallbackTip);
        // 发送统一的渲染提示（如果启用）
        ...
        const sendRenderResult = async (result, type) => {
            ...
            const isImage = type === 'svg' || type === 'puppeteer';
