...
function apply(ctx, config) {
    ...
    * @returns {{useText: boolean, usePuppeteer: boolean, useSvg: boolean, order: string[]}}
    */
   function parseRenderMode() {
      ...
      if (Array.isArray(renderMode) && renderMode.length > 0 && typeof renderMode[0] === 'string') {
         return {
            ...
            order: renderMode.filter(m => ['text', 'svg', 'puppeteer'].includes(m))
         };
      }
      ...
      return { useText, usePuppeteer, useSvg, order: enabledModes };
   }

    // 启动时显示配置提示
    const { useText, usePuppeteer, useSvg, order } = parseRenderMode();
    logger.info(`🎵 music-link 启动 - 当前出图模式：${order.join(' → ') || '无'}`);
    if (useText) {
       logger.info(`📝 纯文本模式已启用`);
    }
    if (usePuppeteer) {
       logger.info(`🎨 Puppeteer 渲染已启用 | 样式：${config.imageStyle} | 暗黑模式：${config.darkMode}`);
    }
    if (useSvg) {
   ...
   // 创建工具函数集合
   const { ensureCacheDir, requestWithProxy, downloadFile } = createUtilFunctions({
      ctx,
      config,
      logger,
      logInfo,
                     ...
                     if (!searchData || !searchData.songs || searchData.songs.length === 0) {
                      ...
                      if (config.verboseFileLog) {
                         try {
                            ...
                            fsSync.writeFileSync(path.join(logDir, 'songlist-latest.json'), JSON.stringify(songList, null, 2));
                         } catch (e) {
                      ...
                      if ( (config.skipSongListSelection || options.skip) ) {
                         ...
                         const formattedList = songList.map((song, index) => {
                            ...
                            if (separator === '\t' && config.smartTabAlignment) {
                               // 使用固定tab数，确保对齐
                               const songNameTabs = 2; // 歌曲名称后2个tab
                               const artistTabs = albumInfo ? 2 : 0; // 歌手后2个tab（如果有专辑）
                         ...
                         if (!useText && !usePuppeteer && !useSvg) {
                            return `${h.quote(session.messageId)}❌ 你没有勾选任何格式的歌单！请去 WebUI 出图模式设置 勾选至少一个 renderMode`;
                         }
                         ...
                         if (strictOrder) {
                            // 严格顺序模式：按 text → svg → puppeteer 顺序执行
                            if (useText) {
                            ...
                            if (useSvg) {
                               ...
                               const svgResult = await generateSongListImage(ctx.puppeteer, formattedList, config, logger, imageStyle, songList).catch(() => null);
                            ...
                            if (useSvg) {
                               const imageStyle = options.image_style ? IMAGE_STYLE_MAP[Object.keys(IMAGE_STYLE_MAP)[options.image_style - 1]] : config.imageStyle;
                               renderPromises.push(
                                  generateSongListImage(ctx.puppeteer, formattedList, config, logger, imageStyle, songList).then(result => {
                     ...
                     if ((config.skipSongListSelection || options.skip) && !options.number) {
                        input = '1';
                         ...
                         const formattedList = songList.map((song, index) => {
                            ...
                            if (separator === '\t' && config.smartTabAlignment) {
                               // 使用固定tab数，确保对齐
                               const platformTabs = 1; // 平台标签后1个tab
                               const songNameTabs = 2; // 歌曲名称后2个tab
                               const artistTabs = albumInfo ? 2 : 0; // 歌手后2个tab（如果有专辑）
