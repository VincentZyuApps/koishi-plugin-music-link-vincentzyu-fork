 ...
 */
function loadFontBase64(fontConfigPath, fallbackFontPath) {
    // 验证路径是否为绝对路径
    if (fontConfigPath && !path.isAbsolute(fontConfigPath)) {
        console.warn(`⚠️ 警告: 字体文件路径不是绝对路径，将被忽略: "${fontConfigPath}"`);
        console.warn('   💡 请使用绝对路径，例如: "/path/to/assets/font.ttf"');
        fontConfigPath = null;
    }
...
function loadImageBase64(imagePath) {
    ...
    if (imagePath && !path.isAbsolute(imagePath)) {
        console.warn(`⚠️ 警告: 背景图片路径不是绝对路径，将被忽略: "${imagePath}"`);
        console.warn('   💡 请使用绝对路径，例如: "/path/to/assets/background.png"');
...
function generateOriginBlackWhiteHtml(listText, config) {
    ...
    const { fontBase64, fontName } = loadFontBase64(config.textFontPath, config.__musicLinkDefaultSourceHanSerifPath);
