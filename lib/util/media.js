...
const { getQualityCandidates, getQualityLabel } = require('./quality');
...
async function advanceQuality(state, log) {
    ...
    while (state.index + 1 < state.candidates.length) {
        ...
        try {
            ...
            if (!nextData?.url) {
                log?.(`音质 ${nextCandidate} 未返回媒体 URL，继续降低音质`);
                continue;
            }
            if (state.seenUrls.has(nextData.url)) {
                log?.(`音质 ${nextCandidate} 返回了重复媒体 URL，跳过重复上传`);
                continue;
            }
            ...
            log?.(`获取音质 ${nextCandidate} 失败，继续降低音质: ${error.message}`);
        }
    }
...
async function sendMediaWithFallback(options) {
        ...
        try {
            ...
            if (state.index > startIndex || state.pendingInitialDowngradeFrom != null) {
                ...
                try {
                    ...
                    log?.(`降级状态提示失败，但媒体已经发送成功: ${error.message}`);
                }
            ...
            const advanced = await advanceQuality(state, log);
            if (!advanced) break;
    ...
    try {
        ...
        log?.(`最终失败状态提示发送失败: ${error.message}`);
    }
