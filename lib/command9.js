            ...
            .action(async ({ session, options }, keyword) => {
                ...
                if (isSongId && !options.number) {
                    ...
                    try {
                        ...
                        // 发送音乐卡片
                        if (config.command9_add_music_card) {
                        ...
                        if ((config.skipSongListSelection || options.skip) && !options.number) {
                            ...
                            const formattedList = songList.map((song, index) => {
                                ...
                                if (separator === '\t' && config.smartTabAlignment) {
                                    ...
                                    // 计算需要的 tab 数量：向上取整到下一个 tab 边界
                                    const platformTabs = Math.ceil((maxPlatformWidth + 1) / tabWidth);
                                    const songNameTabs = Math.ceil((maxSongNameWidth + 1) / tabWidth);
                                    const artistTabs = albumInfo ? Math.ceil((maxArtistWidth + 1) / tabWidth) : 0;
