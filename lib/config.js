...
const Config = Schema.intersect([
    ...
    Schema.object({
        ...
        canvasWidth: Schema.number().default(555).min(300).max(800).step(1).experimental().description('📐 Canvas 图片宽度（px）</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，</br> 😞所以建议暂时不要动本选项捏</i>'),
        canvasScale: Schema.number().default(2.5).min(0.5).max(10).step(0.1).description('🔍 Canvas 内部渲染缩放倍率。值越大越清晰，但耗时和图片体积也会增加'),
        canvasColumns: Schema.number().default(2).min(1).max(4).step(1).experimental().description('📊 Canvas 歌单列数</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，</br> 😞所以建议暂时不要动本选项捏</i>'),
