...
function createNotifierInfo(ctx, config, { useText, usePuppeteer, useSvg, order }) {
   ...
   const infoItems = [];
   infoItems.push(`🎵 当前出图模式：${order.join(' → ') || '无'}`);
...
const Config = Schema.intersect([
   ...
   Schema.object({
      renderMode: Schema
         ...
         .description('🖼️ 出图模式设置（可添加多个，按顺序执行）<br>✨ 配合"严格顺序模式"可实现：文字 → SVG → Puppeteer 的发送顺序<br>⚠️ 如果启用了多个相同模式，只有第一个会生效'),
      strictOrderMode: Schema.boolean().default(false).description('📋 是否严格按照上表顺序执行（默认 false 为并行渲染）'),
   }).description('🖼️ 出图模式设置'),
