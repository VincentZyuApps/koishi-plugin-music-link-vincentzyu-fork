...
exports.Config = exports.usage = exports.IMAGE_STYLE_MAP = exports.platformMap = exports.command9_returnDataField_default = exports.command6_returnDataField_default = void 0;
...
function createNotifierInfoOfConfig(ctx, config, { useText, usePuppeteer, useSvg, useMarkdownTable, useMarkdownStyle, order, selectedNone }) {
...
const Config = Schema.intersect([
    ...
    Schema.object({
        ...
        svgWidth: Schema.number().default(555).min(300).max(800).step(1).experimental().description('📐 SVG 图片宽度（px）</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，</br> 😞所以建议暂时不要动本选项捏</i>'),
        svgColumns: Schema.number().default(2).min(1).max(4).step(1).experimental().description('📊 SVG 歌单列数</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，</br> 😞所以建议暂时不要动本选项捏</i>'),
    ...
    Schema.union([
        ...
        Schema.object({
            ...
            command6_AddOnebotMusicCard: Schema.boolean().default(true).description("🎵 是否发送onebot音乐卡片，位于所有的字段的最后 <br/> *仅适用于 `onebot` 和 `red` 平台，其他平台开启无效*"),
            ...
            })).role('table').description('📊 歌曲返回信息的字段选择<br>[➣ 点我查看该API返回内容示例](http://music.163.com/api/search/get/web?csrf_token=hlpretag=&hlposttag=&s=蔚蓝档案&type=1&offset=0&total=true&limit=10)').default(command6_returnDataField_default),
        }).description('🎵 各种`网易点歌`的API返回设置'),
        ...
        Schema.object({
            ...
            command9_AddOnebotMusicCard: Schema.boolean().default(false).description("🎵 是否发送onebot音乐卡片，位于所有的字段的最后 <br/> *仅适用于onebot平台，其他平台开启无效*"),
    ...
    Schema.object({
        ...
        dataFieldSortMode: Schema.union([
            ...
            Schema.const('raw').description('严格按照 `command_returnDataField` 表格的顺序 （严格按照配置项表格的上下顺序）'),
    ...
    Schema.object({
        ...
        renameTempFile: Schema.boolean().default(false).description('✏️ 是否启用`音频文件`自定义命名<br>关闭则使用随机十六进制字符串作为文件名（`crypto.randomBytes(8).toString(\'hex\')`）<br>💡 <i>仅在上方"【启用文件下载】`enableFileDownload`"配置项开启时生效</i>'),
