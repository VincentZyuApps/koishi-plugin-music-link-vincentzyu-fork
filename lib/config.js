...
const Config = Schema.intersect([
    ...
    // 🔍 调试模式
    Schema.object({
        verboseConsoleLog: Schema.boolean().default(false).description('🐛 控制台详细日志开关'),
        verboseFileLog: Schema.boolean().default(false).description('📝 开启后将最后一次歌单完整写入 `/log/songlist-latest.json`；控制台只输出摘要（长度 + 前50字符），避免刷屏'),
    }).description('🔍 调试模式'),
]);
