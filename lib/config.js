...
const { getDefaultAssetDisplayPath } = require('./util');
const { QUALITY_PROFILES } = require('./util/quality');
const { TELEGRAM_CAPTION_MODES } = require('./platform/telegram/send');
...
const command6_returnDataField_default = [
    ...
    { "data": "pic", "describe": "🖼️ 封面链接", "type": "image" },
...
function createTelegramCaptionModeSchema() {
    return Schema.union([
        Schema.const(TELEGRAM_CAPTION_MODES.LEGACY).description('保持原有行为：图片与全部文本合并发送，长歌词可能超过 caption 限制'),
        Schema.const(TELEGRAM_CAPTION_MODES.SEPARATE_LYRICS).description('歌词单独发送：图片与其他文本合并，歌词按 3333 字符拆分（默认）'),
        Schema.const(TELEGRAM_CAPTION_MODES.SEPARATE_IMAGE).description('图片单独发送：图片独立发送，全部文本按 3333 字符拆分（最稳妥）'),
    ]).role('radio').default(TELEGRAM_CAPTION_MODES.SEPARATE_LYRICS)
        .description('✈️ Telegram 图文发送策略<br>仅影响 Telegram，其他平台保持原有行为');
}
...
function createCommand9QualitySchemas() {
    return Object.fromEntries(Object.values(QUALITY_PROFILES).map((profile) => [
        profile.configKey,
        Schema.union(profile.qualities.map((quality) => (
            Schema.const(quality.value).description(quality.label)
        )))
            .description(profile.configDescription)
            .default(profile.defaultQuality),
    ]));
}
...
const Config = Schema.intersect([
    ...
    Schema.object({
        ...
        strictOrderMode: Schema.boolean().experimental().default(true).description('📋 是否严格按照上表顺序串行执行（默认 true 为串行渲染）<br>⚠️ <b>强烈建议开启</b>：由于 Node.js单线程特性，并行渲染会导致 CPU争抢，使所有模式都变慢<br>✅ 串行模式：各渲染器独占CPU，互不干扰，总耗时更短<br>🐢 并行模式：多个渲染器同时竞争单线程CPU，互相阻塞，性能会有下降'),
    ...
    Schema.object({
        ...
        backgroundImagePath: Schema.string().role('textarea', { rows: [2, 5] }).default(getDefaultAssetDisplayPath('pixai_koishi.png')).description(`🎨 背景图片路径. 仅对${IMAGE_STYLE_MAP.MODERN_SOURCE_HANS_SERIF}生效<br>📁 默认展示 process.cwd()/data/assets/music-link-vincentzyu-fork；运行时自动使用 ctx.baseDir/data/assets/music-link-vincentzyu-fork`),
        textFontPath: Schema.string().role('textarea', { rows: [2, 5] }).default(getDefaultAssetDisplayPath('LXGWWenKaiMono-Regular.ttf')).description('🔤 文字字体文件路径. 对任何 imageStyle 都生效。<br>📁 默认展示 process.cwd()/data/assets/music-link-vincentzyu-fork；运行时自动使用 ctx.baseDir/data/assets/music-link-vincentzyu-fork'),
    ...
    Schema.object({
        ...
        svgFontFiles: Schema.array(Schema.string())
            .role('table')
            .default([
                getDefaultAssetDisplayPath('LXGWWenKaiMono-Regular.ttf'),
                // getDefaultAssetDisplayPath('SourceHanSerifSC-Medium.otf'),
            ])
            .description('🔤 resvg 渲染使用的字体文件路径 <b>(绝对路径)</b>，会按顺序查找所有存在的字体文件并加载<br>📁 默认展示 process.cwd()/data/assets/music-link-vincentzyu-fork；运行时自动使用 ctx.baseDir/data/assets/music-link-vincentzyu-fork'),
    ...
    Schema.object({
        ...
        canvasFontFiles: Schema.array(Schema.string())
            ...
            .description('🔤 Canvas 渲染使用的字体文件路径 <b>(绝对路径)</b>，会按顺序查找所有存在的字体文件并加载<br>📁 默认展示 process.cwd()/data/assets/music-link-vincentzyu-fork；运行时自动使用 ctx.baseDir/data/assets/music-link-vincentzyu-fork'),
    ...
    Schema.union([
        ...
        Schema.object({
            ...
            command6_AddOnebotMusicCard: Schema.boolean().default(true).description("🎵 是否发送 OneBot 音乐卡片，位于所有字段的最后<br/>✅ 默认开启，仅适用于 `onebot` 平台，其他平台开启无效"),
            command6_AddQqRichuiCard: Schema.boolean().experimental().default(true).description("🎴 是否在歌曲字段前发送 QQ RichUI 音乐卡片<br/>✅ 默认开启，仅适用于 QQ 官方机器人平台，依赖客户端 RichUI 能力"),
            ...
            command6_telegramCaptionMode: createTelegramCaptionModeSchema(),
        ...
        Schema.object({
            ...
            command9_luoyueApiBaseUrl: Schema.string().role('link').default('https://api.vkeys.cn').description('🔗 落月 API 的基础 URL <i> (默认使用作者自建 API 地址，也可以替换为其他自建或镜像地址</i> <br>🧩 官方 API：`https://api.vkeys.cn`<br> 🌐 作者自建 API：`http://xwl.vincentzyu233.cn:51217` <br> <b>⚠️`vkey`官方不支持酷狗, `vincentzyu`自建支持酷狗 </b> <br> <i>💬 如果挂了可以去qq群`1085190201`里艾特我 `@VincentZyu` </i>'),
            ...
            ])).role('checkbox').default(['netease', 'tencent', 'kugou']).description('🎧 选择音乐平台<br>✨ 多选即聚合，不再单独保留旧版本的 `aggregation`选项'),
            command9_searchListLength: Schema.number().default(60).min(1).max(100).description('📋 歌曲搜索的列表长度，即返回的候选项个数<br>🧮 如果选了多个音乐平台，则每个平台的请求数量 = `ceil(Len / n)`<br> 🔢 其中 `Len` 是这里填写的值，`n` 是勾选的平台数, `ceil`是指向上取整 <br>📌 例如 `Len=50`、`n=3` 时：`50/3 = 16.666... ≈ 17`<br>💡 默认 `Len=60` 时，`n=1、2、3` 均可整除，每个平台分别请求 `60、30、20` 首'),
            command9_maxDuration: Schema.natural().description('⏳ 歌曲最长持续时间，单位为：秒').default(1800),
            ...createCommand9QualitySchemas(),
            command9_AutoDowngradeQuality: Schema.boolean().experimental().default(true).description('🛟 媒体发送失败时是否自动逐级降低音质重试<br>✅ 开启（默认）：从当前所选音质逐级尝试到最低音质<br>❌ 关闭：不降低音质，但单个媒体失败仍不会影响图文及其他媒体发送'),
            command9_AddOnebotMusicCard: Schema.boolean().default(true).description("🎵 是否发送 OneBot 音乐卡片，位于所有字段的最后<br/>✅ 默认开启，仅适用于 `onebot` 平台，其他平台开启无效"),
            command9_AddQqRichuiCard: Schema.boolean().experimental().default(true).description("🎴 是否在歌曲字段前发送 QQ RichUI 音乐卡片<br/>✅ 默认开启，仅适用于 QQ 官方机器人平台，依赖客户端 RichUI 能力"),
            ...
            command9_telegramCaptionMode: createTelegramCaptionModeSchema(),
    ...
    // 💬 更多Bot发送的音乐消息细节设置
    Schema.object({
        isfigure: Schema.boolean().default(false).description("📦 `图片、文本`元素使用合并转发，其余单独发送<br>仅支持 `onebot`、`red` 平台，其他平台开启无效"),
    ...
    Schema.object({
        ...
        middlewareDefaultSongIndex: Schema.number().default(1).min(0).max(10).description("🔢 在歌单里默认选择的序号<br>范围`0-10`，无需考虑11-20，会自动根据Onebot音乐卡片卡片的平台选择。若音乐平台不匹配 则在搜索项前十个进行选择。"),
    ...
    Schema.object({
        ...
        fileNameSlashReplacement: Schema.union([
            ...
            Schema.const('_').description('📁 下划线 _（路径友好）'),
            Schema.const(',').description('📝 逗号 ,（更符合日常阅读习惯）'),
            Schema.const('').description('✂️ 直接删除（最紧凑）'),
        ]).default('-').role('radio').description('📁 文件名中的斜杠`/`替换为什么字符<br>影响歌曲名、歌手名等所有字段中的斜杠<br>⚠️ 建议使用默认的横杠`-`，这是业界通用做法，可避免路径解析等奇怪问题<br>💡 <i>需同时开启"【启用文件下载】`enableFileDownload`"和"【自定义命名】`renameTempFile`"才生效</i>').experimental(),
        fileTransferMode: Schema.union([
            Schema.const('localPath').description('📁 本地路径 file:// 协议（适用于同一设备或共享文件目录）'),
            Schema.const('base64').description('📦 Base64 base64:// 协议（适用于不同设备或容器）'),
        ]).role('radio').default('base64').description('📡 文件传输模式<br>仅影响发送文件时使用的协议类型，不影响文件是否保存到本地磁盘<br>⚠️ Koishi 与协议端位于同一设备或共享文件目录时建议使用 `localPath`；位于不同设备或容器时建议使用 `base64`<br>💡 <i>无论选择哪种模式，只要开启"【启用文件下载】`enableFileDownload`"就会保存到本地磁盘</i>').experimental(),
    }).description('⬇️ 音乐文件下载设置 📄'),

    // 🔍 调试模式
    Schema.object({
        verboseConsoleLog: Schema.boolean().default(false).description('🐛 控制台详细日志开关<br>开启后额外输出 API URL、请求 payload、原始响应、内部执行过程和 Stack trace；关闭时仍保留业务结果、降级动作及错误摘要'),
        verboseFileLog: Schema.boolean().default(false).description('📝 开启后将最后一次歌单完整写入 `/log/songlist-latest.json`；控制台常驻日志只输出歌曲数量和文件路径，短预览仅在详细日志中出现'),
    }).description('🔍 调试模式'),
]);
