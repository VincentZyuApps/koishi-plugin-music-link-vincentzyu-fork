    ...
    Schema.union([
        ...
        Schema.object({
            ...
            command9_NeteaseMusicQuality: Schema.union([
                Schema.const(1).description('标准（64k，standard）'),
                Schema.const(2).description('标准（128k，standard）'),
                Schema.const(3).description('HQ极高（192k，higher）'),
                Schema.const(4).description('HQ极高（320k，exhigh）'),
                Schema.const(5).description('SQ无损（lossless）'),
                Schema.const(6).description('高解析度无损（Hi-Res，hires）'),
                Schema.const(7).description('高清臻音（Spatial Audio，jyeffect）'),
                Schema.const(8).description('沉浸环绕声（Surround Audio，sky）'),
                Schema.const(9).description('超清母带（Master，jymaster）'),
            ]).description('🎼 网易云音乐最大音质（网易云音乐专用）').default(5),
            command9_QQMusicQuality: Schema.union([
                Schema.const(4).description('标准音质（128）'),
                Schema.const(8).description('HQ高音质（320）'),
                Schema.const(10).description('SQ无损音质（flac）'),
                Schema.const(11).description('Hi-Res音质（hires）'),
                Schema.const(12).description('杜比全景声（dolby）'),
                Schema.const(14).description('臻品母带2.0（master）'),
            ]).description('🎼 QQ 音乐最大音质（QQ 音乐专用）').default(10),
            command9_KugouMusicQuality: Schema.union([
                Schema.const('128').description('标准音质（128）'),
                Schema.const('320').description('HQ高品质（320）'),
                Schema.const('flac').description('SQ无损（flac）'),
                Schema.const('high').description('Hi-Res（high）'),
