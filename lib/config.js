...
const Config = Schema.intersect([
   ...
   Schema.object({
      renderMode: Schema
         ...
         .description('🖼️ 消息渲染模式设置（可添加多个，按顺序执行）<br>✨ (非qq官机平台)配合"严格顺序模式"可实现：文字 → SVG → Puppeteer 的发送顺序<br>⚠️ 如果启用了多个相同模式，只有第一个会生效<br>⚠️ QQ官机平台建议一次只开启一个，防止发生未知问题。如果想要支持QQ官机平台的多种模式同时发送，可以去GitHub提issue/pr或者群里和我讨论捏（因为我自己也没完全研究明白hhh'),
