...
function createNotifierInfo(ctx, config, { useText, usePuppeteer, useSvg, useMarkdownTable, useMarkdownStyle, order }) {
   if (!ctx.notifier) return;
   
   const { h } = require('koishi');
   const infoItems = [];
   infoItems.push(`🎵 当前消息渲染模式：${order.join(' → ') || '无'}`);
   infoItems.push(`⚡ 渲染模式：${config.strictOrderMode ? '严格顺序' : '并行'}模式`);
...
function createNotifierAssets(ctx, downloadStatus) {
...
exports.createNotifierInfo = createNotifierInfo;
exports.createNotifierAssets = createNotifierAssets;
...
const Config = Schema.intersect([
   Schema.object({
      ...
      useRealAtRobot: Schema.boolean().default(true).experimental().description("🤖 开启后 发送歌单消息时使用真实的艾特机器人自身的消息段，而不是固定的@机器人文本"),
   ...
   Schema.object({
      renderMode: Schema
         ...
         .description('🖼️ 消息渲染模式设置（可添加多个，按顺序执行）<br>✨ 配合"严格顺序模式"可实现：文字 → SVG → Puppeteer 的发送顺序<br>⚠️ 如果启用了多个相同模式，只有第一个会生效'),
      strictOrderMode: Schema.boolean().default(false).description('📋 是否严格按照上表顺序执行（默认 false 为并行渲染）'),
   }).description('🖼️ 渲染模式设置'),
   ...
   Schema.object({
      ...
      svgScale: Schema.number().default(3.3).min(0.1).max(10).step(0.1).description('🔍 SVG 渲染缩放比例（影响输出图片大小和清晰度）'),
      svgWidth: Schema.number().default(555).min(300).max(800).step(1).experimental().description('📐 SVG 图片宽度（px）</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，建议不要动本选项</i>'),
      svgColumns: Schema.number().default(2).min(1).max(4).step(1).experimental().description('📊 SVG 歌单列数</br><i>⚠️ 调整列数 和 宽度 可能导致格式变的比较乱，暂时还没想到太好的智能排版方案，建议不要动本选项</i>'),
      svgShowRenderInfo: Schema.boolean().default(true).description('📊 是否在 SVG 图片消息段后 增加文字消息段显示渲染耗时等统计信息'),
   ...
   Schema.object({
      ...
      puppeteerShowRenderInfo: Schema.boolean().default(true).description('📊 是否在 Puppeteer 图片消息段后 增加文字消息段显示渲染耗时等统计信息'),
   }).description('🖌️ Puppeteer 渲染图片歌单设置').disabled(config => 
      !config.renderMode?.includes('puppeteer')
   ),

   Schema.object({
      markdownTableMaxDisplay: Schema.number().default(20).min(1).max(50).description('📊 QQ Markdown 表格模式显示的最大歌曲数量'),
      markdownStyleMaxDisplay: Schema.number().default(10).min(1).max(50).description('💫 QQ Markdown 格式风格显示的最大歌曲数量'),
   }).description('📝 QQ Markdown 歌单设置').disabled(config => 
      !config.renderMode?.some(m => m.mode === 'markdown_table' || m.mode === 'markdown_style')
   ),
   ...
   Schema.union([
      ...
      Schema.object({
         ...
         command6_usedAPI: Schema.union([
            Schema.const('api.injahow.cn').description('黑胶只能30秒的`api.injahow.cn`后端（适合官方bot）'),
   ...
   Schema.object({
      ...
      renameTempFile: Schema.boolean().default(false).description('✏️ 是否启用`音频文件`自定义命名<br>关闭则使用随机hash值'),
      fileNameTemplate: Schema.string().role('textarea', { rows: [2, 4] }).default('${name}-${artist}-${time}').description('📄 文件名模板（不含扩展名，扩展名会自动添加）<br>可用占位符：<br>`${name}` 歌曲名称<br>`${artist}` 歌手<br>`${id}` 歌曲ID<br>`${quality}` 音质<br>`${platform}` 平台<br>`${time}` 时间戳 (YYYYMMDD-HHMMSS)'),
      fileNameKeepSpaces: Schema.boolean().default(false).description('🔤 是否保留文件名中的空格<br>⚠️ 关闭（默认）：空格会被替换为横杠`-`，这是业界通用做法，可避免某些系统/协议端的兼容性问题').experimental(),
      ...
      ]).default('-').description('📁 文件名中的斜杠`/`替换为什么字符<br>影响歌曲名、歌手名等所有字段中的斜杠<br>⚠️ 建议使用默认的横杠`-`，这是业界通用做法，可避免路径解析等奇怪问题').experimental(),
      deleteTempTime: Schema.number().default(300).description('🗑️ 对于`file`类型的`Temp`临时文件的删除时间<br>若干`秒`后 删除下载的本地临时文件<br>⚠️ 设置为 `0` 或 `负数` 时，表示永远不删除临时文件'),
      ...
      ]).role('radio').default('base64').description('📡 文件传输模式<br>`base64`模式可解决跨设备传输时协议端无法访问本地路径的问题').experimental(),
   }).description('🔧 高级进阶设置'),
